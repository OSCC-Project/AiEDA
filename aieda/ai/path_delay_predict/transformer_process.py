...
class YMLDatasetHandler:
    ...
    def collect_yml(self) -> Dict[str, int]:
        ...
        for root, dirs, files in os.walk(self.data_from):
            if os.path.basename(root) == "wire_paths":  # Only process wire_paths directories
                for file in files:
                    if file.endswith('.yml'):  # Filter .yml files
                        relative_path = os.path.relpath(root, self.data_from)
                        yml_file_counts[relative_path] = yml_file_counts.get(relative_path, 0) + 1
    ...
    def sample_dataset(self, sample_num: int, seed: int = 42) -> List[str]:
        ...
        if len(file_paths) < sample_num:
            print(f"Warning: Insufficient files, only {len(file_paths)} available. Using all available files.")
...
class CircuitParser:
    ...
    @staticmethod
    def pad_tensors(tensor_list: List[torch.Tensor], max_length: int) -> List[torch.Tensor]:
...
class DataProcessor:
    ...
    def process_dataset_dirs(
        ...
        for data_from in dirs_list:
            ...
            sampled_files = handler.sample_dataset(
                sample_num, 
            ...
            if results:  # Ensure we have results
                ...
                if normalize:
                    ...
                    normalization_stats[design_name] = {
                        "mean": mean,
                        "std": std
                    }
    ...
    def prepare_data(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
    ...
    def denormalize_predictions(
        ...
        if design_name not in stats:
            raise ValueError(f"Cannot find normalization statistics for design {design_name}")
